      ...
      {open && (
        <>
          ...
          >
            ...
            <div className="flex-1 overflow-y-auto px-3 py-4">
              {NAV.map((item) => (
                <Link
                  ...
                  to={item.to}
                  onClick={onClose}
                  className="flex items-center justify-between rounded-2xl px-4 py-3.5 text-base font-semibold text-graphite-800 hover:bg-pine-50 hover:text-pine-700"
                >
                  {t(item.key)}
                </Link>
              ))}
              ...
              <div className="mt-4 border-t border-graphite-100 px-4 pt-4">
                <p className="mb-2 text-[11px] font-bold uppercase tracking-wider text-graphite-400">{t('common.toggle')}</p>
                <div className="flex gap-2">
                  {(['ru', 'tj', 'en'] as const).map((l) => (
                    <button
                      ...
                      onClick={() => setLang(l)}
                      className={cn(
                        'flex-1 rounded-xl border py-2 text-sm font-bold uppercase transition-colors',
                ...
                <p className="mb-2 mt-4 text-[11px] font-bold uppercase tracking-wider text-graphite-400">Валюта</p>
                <div className="flex gap-2">
                  {(['TJS', 'USD', 'EUR'] as const).map((c) => (
                    <button
                      ...
                      className={cn(
                        'flex-1 rounded-xl border py-2 text-sm font-bold transition-colors',
            ...
            <div className="border-t border-graphite-100 p-4 pb-safe">
              {user ? (
                <div className="space-y-2">
                  <div className="flex items-center gap-3 px-1">
                    <span className="grid h-10 w-10 place-items-center rounded-full bg-pine-100 font-bold text-pine-700">
                ...
                <div className="space-y-2">
                  <Link to="/auth" onClick={onClose} className="btn-primary w-full py-3">
                    {t('auth.signIn')}
                  </Link>
                  <Link to="/favorites" onClick={onClose} className="btn-outline w-full py-3">
